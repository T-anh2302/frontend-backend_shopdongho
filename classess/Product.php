...
if ($productName == "" || $catId == "" || $brandId == "" || $body == "" || $price == "" || $file_name == "" || $type == "") {
	...
	$msg = "<span class='error'>Các trường nhập không được để trống !</span>";
	return $msg;
}elseif ($file_size >1048567) {
     echo "<span class='error'>Kích thước ảnh nên nhỏ hơn 1MB!
     </span>";
...
if ($productName == "" || $catId == "" || $brandId == "" || $body == "" || $price == "" ||$type == "") {
	...
	$msg = "<span class='error'>Các trường không được để trống !</span>";
	return $msg;
...
if ($file_size >1048567) {
	 ...
	 $updatedted_row = $this->db->update($query);
			if ($updatedted_row) {
				$msg = "<span class='success'>Sản phẩm cập nhật thành công.</span>";
				return $msg;
			} else{
...
public function insertCompareData($cmprid,$cmrId){
	...
	if ($check) {
		$msg = "<span class='error'>Sản phẩm đã được thêm !</span>";
				return $msg;
	}
	...
	$msg = "<span class='success'>Thêm thành công vào trang so sánh để kiểm tra</span>";
				return $msg;
	}else{
...
public function saveWishListData($id,$cmrId){
	...
	$check = $this->db->select($cquery);
	if ($check) {
		$msg = "<span class='error'>Đã tồn tại sản phẩm. Vui lòng chọn sản phẩm khác !</span>";
				return $msg;
	}
	...
	$msg = "<span class='success'>Đã được thêm ! Kiểm tra trang sản phẩm yêu thích</span>";
		return $msg;
	}else{
   $msg = "<span class='error'>Không thể thêm được!</span>";
		return $msg;
	}
 }
}
